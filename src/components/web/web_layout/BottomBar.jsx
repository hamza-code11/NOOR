...
const BottomBar = () => {
  ...
  const handleMouseLeave = () => {
    if (isClickOpen.current) return; // Don't close if opened by click
    dropdownTimeout.current = setTimeout(() => { setActiveDropdown(null); }, 200);
  };
  ...
  // Toggle dropdown on click (also navigates if no dropdown)
  const handleCategoryClick = (category, e) => {
    if (activeDropdown === category.name) {
      // If already open, close it and navigate
      ...
      e.preventDefault(); // Prevent navigation when opening dropdown
    }
  };

  // Navigate on category name click (when dropdown is open)
          ...
          <div className="hidden lg:flex items-center gap-1 flex-1 ml-3">
            {bottomBarCategories.map((category, index) => (
              <div
                ...
                onMouseLeave={handleMouseLeave}
              >
                {/* Category Button - Click to navigate, Hover for dropdown */}
                <div className="flex items-center">
                  {/* Navigate link - clicking category name redirects */}
                  <a
                    href={category.link || '#'}
                    onClick={(e) => {
                      if (activeDropdown === category.name) {
                ...
                <div key={index} className="border-b border-white/5 last:border-0">
                  <div className="flex items-center">
                    <a 
                      href={category.link || '#'} 
                      onClick={() => setMobileMenuOpen(false)}
                      className="flex-1 p-3 rounded-lg font-medium text-white/80 hover:text-white hover:bg-white/10 transition-colors text-sm"
                    >
                      {category.name}
                    </a>
                    <button 
                    ...
                    >
                      <ChevronDown size={16} className={`transition-transform duration-200 ${mobileExpanded === category.name ? 'rotate-180 text-gold' : ''}`} />
                    </button>
                  </div>
